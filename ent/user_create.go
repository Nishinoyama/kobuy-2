...
func (uc *UserCreate) AddProvidedGroceryIDs(ids ...int) *UserCreate {
	uc.mutation.AddProvidedGroceryIDs(ids...)
	return uc
}

// AddProvidedGroceries adds the "provided_groceries" edges to the Grocery entity.
func (uc *UserCreate) AddProvidedGroceries(g ...*Grocery) *UserCreate {
	ids := make([]int, len(g))
	for i := range g {
		ids[i] = g[i].ID
	}
