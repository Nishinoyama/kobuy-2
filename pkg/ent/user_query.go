...
// QueryProvidedGroceries chains the current query on the "provided_groceries" edge.
func (uq *UserQuery) QueryProvidedGroceries() *GroceryQuery {
	query := (&GroceryClient{config: uq.config}).Query()
...
// WithProvidedGroceries tells the query-builder to eager-load the nodes that are connected to
// the "provided_groceries" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithProvidedGroceries(opts ...func(*GroceryQuery)) *UserQuery {
	query := (&GroceryClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withProvidedGroceries; query != nil {
...
func (uq *UserQuery) loadProvidedGroceries(ctx context.Context, query *GroceryQuery, nodes []*User, init func(*User), assign func(*User, *Grocery)) error {
	...
	for _, n := range neighbors {
		fk := n.user_provided_groceries
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_provided_groceries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
