...
func (lq *LedgerQuery) QueryReceiver() *UserQuery {
	query := (&UserClient{config: lq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := lq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (lq *LedgerQuery) WithReceiver(opts ...func(*UserQuery)) *LedgerQuery {
	query := (&UserClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withReceiver = query
	return lq
}
...
func (lq *LedgerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ledger, error) {
	...
	if query := lq.withReceiver; query != nil {
		if err := lq.loadReceiver(ctx, query, nodes, nil,
			func(n *Ledger, e *User) { n.Edges.Receiver = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LedgerQuery) loadReceiver(ctx context.Context, query *UserQuery, nodes []*Ledger, init func(*Ledger), assign func(*Ledger, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Ledger)
	for i := range nodes {
